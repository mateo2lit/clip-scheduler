...
function XPreview({ title, description, hashtags, videoPreviewUrl, thumbnailPreview, profileName, avatarUrl }: {
  ...
  const displayName = profileName ? profileName.replace(/^@/, "") : "yourhandle";
  const handle = `@${displayName}`;
  const caption = (description || title || "Your caption will appear here…").slice(0, 280);
  const tags = hashtags.slice(0, 3).map((t) => `#${t}`).join(" ");
    ...
    <div className="overflow-hidden rounded-xl border border-white/10 bg-[#000000]">
      <div className="flex gap-2.5 p-3 pb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {caption && <p className="mt-1 text-sm text-white/85 line-clamp-4 leading-snug">{caption}</p>}
          {tags && <p className="mt-0.5 text-xs text-[#1d9bf0]">{tags}</p>}
          {(videoPreviewUrl || thumbnailPreview) && (
            <div className="relative mt-2 overflow-hidden rounded-2xl border border-white/8 bg-black" style={{ aspectRatio: "16/9" }}>
              {videoPreviewUrl ? (
                <video src={videoPreviewUrl} className="h-full w-full object-cover" muted />
              ) : (
                <img src={thumbnailPreview!} alt="" className="h-full w-full object-cover" />
          ...
          {!videoPreviewUrl && !thumbnailPreview && (
            <div className="mt-2 flex h-24 items-center justify-center rounded-2xl border border-white/8 bg-white/[0.03]">
              <svg className="h-8 w-8 text-white/15" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
            </div>
          )}
          <div className="mt-2.5 flex items-center justify-between max-w-[280px]">
            {/* Reply */}
            <div className="flex items-center gap-1 text-white/35">
              <svg style={{ width: 17, height: 17 }} fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
              </svg>
              <span className="text-xs">0</span>
            </div>
      ...
      <div className="border-t border-white/5 px-3 py-2">
        <span className="text-xs text-white/20">0 views</span>
      </div>
    </div>
  );
}
