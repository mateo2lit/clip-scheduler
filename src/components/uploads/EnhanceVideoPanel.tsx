...
import {
  PencilSimple, CaretDown, CaretRight, TextT, TextAlignLeft, Image as ImageIcon,
  Check, X as XIcon,
          ...
          <div className="flex border-t border-white/[0.06]">
            ...
            <div className="flex-1 min-w-0 overflow-y-auto px-4 py-3 space-y-3" style={{ maxHeight: 480 }}>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <AddButton
                  icon={<TextT className="w-3.5 h-3.5" weight="bold" />}
                  label="Title"
                  onClick={() => addLayer({ ...DEFAULT_TITLE_LAYER })}
                />
                <AddButton
                  icon={<TextAlignLeft className="w-3.5 h-3.5" weight="bold" />}
                  label="Text"
                  onClick={() => addLayer({ ...DEFAULT_TEXT_LAYER })}
                />
                <AddButton
                  icon={<ImageIcon className="w-3.5 h-3.5" weight="duotone" />}
          ...
          <div className="border-t border-white/[0.06] px-4 pb-4 pt-3 space-y-2">
            ...
            {burnStatus === "done" && (
              <div className="flex items-center gap-2 rounded-lg bg-green-500/10 border border-green-500/20 px-3 py-2">
                <Check className="w-3.5 h-3.5 flex-shrink-0 text-green-400" weight="bold" />
                <span className="text-xs text-green-300">Burn complete — schedule the enhanced video below</span>
              </div>
            )}
            {burnStatus === "failed" && (
              <div className="flex items-center gap-2 rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2">
                <XIcon className="w-3.5 h-3.5 flex-shrink-0 text-red-400" weight="bold" />
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.02] overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2.5">
        <button type="button" onClick={() => setExpanded((v) => !v)} className="flex-1 flex items-center gap-2 text-left min-w-0">
          <CaretRight className={`w-3 h-3 text-white/30 flex-shrink-0 transition-transform ${expanded ? "rotate-90" : ""}`} weight="bold" />
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={onToggleMore} className="text-[11px] text-white/30 hover:text-white/50 transition-colors flex items-center gap-1">
        <CaretDown className={`w-3 h-3 transition-transform ${showMore ? "rotate-180" : ""}`} weight="bold" />
